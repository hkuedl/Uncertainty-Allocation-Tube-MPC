"""MPC with perfect future information."""
...
class PerfectPredictionMPC:
    """Ground-truth MPC benchmark."""

    def __init__(self, config, physical_params: PhysicalParams | None = None):
        self.config = config
        self.params = physical_params or PhysicalParams()

    def solve(self, real_data, price_data, t0: int, ess0: float, tau0: float, method=None):
        """Solve the perfect-prediction MPC benchmark."""
        import cvxpy as cp
        ...
        tau_forecast = real_temp[t0 : t0 + horizon].reshape(-1)
        p_forecast = real_net_load[t0 : t0 + horizon].reshape(-1)
        ...
        for t in range(horizon):
            ...
            constraints.append(tau_hat[t] <= params.tau_max[t0 + t])
            constraints.append(tau_hat[t] >= params.tau_min[t0 + t])
