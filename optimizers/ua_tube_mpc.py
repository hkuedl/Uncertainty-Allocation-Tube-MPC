"""Uncertainty-aware tube-based MPC solvers."""
...
class UATubeMPC:
    """Hard- and soft-tube UA-MPC entry points."""
    ...
    def solve_hard_tube(
        ...
        """Solve UA tube MPC with hard tubes."""
    ...
    def solve_soft_tube(
        ...
        """Solve UA tube MPC with slack variables."""
        return self._solve_tube(
            x_data=x_data,
            price_data=price_data,
            t0=t0,
    ...
    def _solve_tube(
        ...
        elec_w = [
            dr_bounds(params.www * p_scens[:, h], weights, beta=beta) for h in range(horizon)
        ]
        temp_w = [
            dr_bounds(params.www * tau_scens[:, h], weights, beta=beta) for h in range(horizon)
        ]
        elec_epi = np.array([radius(p_scens[:, h], beta=beta) for h in range(horizon)])

        alph_buy = cp.Variable(horizon, nonneg=True)
        alph_ess = cp.Variable(horizon, nonneg=True)
        alph_hvac = cp.Variable(horizon, nonneg=True)
        alph_pvc = cp.Variable(horizon, nonneg=True)
        ...
        cc = cp.multiply(alph_buy, c_buy[t0 : t0 + horizon]) - cp.multiply(alph_pvc, c_cur)
        lam_obj = cp.Variable(horizon)
        s_obj = cp.Variable((horizon, n_samples))
        ...
        constraints.append(alph_buy + alph_ess + alph_hvac + alph_pvc == np.ones(horizon))
        ...
        s_ess_lb = cp.Variable(horizon)
        s_ess_ub = cp.Variable(horizon)
        ...
        for t, bounds in enumerate(elec_w):
            # bounds is the tuple of (lower_bound, upper_bound) of net load
            e_lb, e_ub = bounds 
            ...
            if t == 0:
                constraints.append(s_ess_lb[t] == alph_ess[t] * soc_error_lb)
                constraints.append(s_ess_ub[t] == alph_ess[t] * soc_error_ub)
            else:
                constraints.append(s_ess_lb[t] == phi_ess * s_ess_lb[t - 1] + alph_ess[t] * soc_error_lb)
                constraints.append(s_ess_ub[t] == phi_ess * s_ess_ub[t - 1] + alph_ess[t] * soc_error_ub)
        ...
        s_temp_lb = cp.Variable(horizon)
        s_temp_ub = cp.Variable(horizon)
        ...
        ther_ps = [
            ...
            p_heat.value[0],
            p_cool.value[0],
            params.p_hvac_max - k_temp_heat * s_temp_ub.value[0],
            params.p_hvac_max + k_temp_cool * s_temp_lb.value[0],
        ]
        ther_taus = [
            tau.value[0],
            params.tau_max[t0] - s_temp_ub.value[0],
            params.tau_min[t0] - s_temp_lb.value[0],
        ]
        ess_ps = [
            p_ess_hat.value[0],
            p_ch.value[0],
            p_dis.value[0],
            params.p_ch_max - k_ess_ch * s_ess_ub.value[0],
            params.p_dis_max + k_ess_dis * s_ess_lb.value[0],
        ]
        ess_esss = [
            ess.value[0],
            params.ess_max - s_ess_ub.value[0],
            params.ess_min - s_ess_lb.value[0],
        ]
